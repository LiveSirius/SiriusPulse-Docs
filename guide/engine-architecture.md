...
>
> **动画表情缓存命中快速拦截**：在感知阶段完成后，引擎会检查消息是否为纯动画表情消息，并且该表情的 caption 是否已在缓存中命中。如果是，则直接返回 `SILENT` 策略，不再进行后续的认知分析、决策和执行，实现零 LLM 成本的快速响应，同时将缓存的 caption 更新到 basic_memory 中。

## 核心子系统

### 认知分析器（CognitionAnalyzer）

联合分析情绪和意图。输入最近 N 条历史消息 + 当前消息，输出 `IntentAnalysisV3` + `EmotionState`。`IntentAnalysisV3` 包含 `sticker_caption`（动画表情缓存描述）和 `image_caption`（普通图片描述）字段。在回写 basic_memory 时，引擎优先使用 `sticker_caption`（如果存在）；对于动画表情消息，会移除无意义的文件哈希，替换为 `【动画表情：caption】` 格式；对于普通图片则使用 `【图片】【图片描述：caption】` 格式。对于普通图片消息，如果所有图片都已解析且没有需要首次分析的多模态项目，则跳过 LLM 调用，直接使用规则分析。对于纯动画表情消息，当缓存命中时会直接在感知阶段后跳过整个管线（详见“5 阶段管线”中的动画表情缓存命中快速拦截），不会进入认知分析器。
